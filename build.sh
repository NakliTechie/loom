...
SRC="k_standard.c s_scalbn.c s_scalbnf.c s_ldexp.c s_ldexpf.c e_fmod.c e_remainder.c e_sqrt.c w_remainder.c w_sqrt.c \
     arithmetic.c fparithmetic.c netcfg.c shlink.c server.c p.c main.c webterm.c webfpu.c web.c"
emcc -O2 -I. -Wno-unused -Wno-format \
  ...
  $SRC -lm \
  -sMODULARIZE=1 -sEXPORT_ES6=1 -sEXPORT_NAME=createT4 -sENVIRONMENT=$ENV \
  -sALLOW_MEMORY_GROWTH=1 -sINITIAL_MEMORY=16MB -sEXIT_RUNTIME=0 -sINVOKE_RUN=0 \
  -sFORCE_FILESYSTEM=1 \
  -sEXPORTED_FUNCTIONS=_main,_t4_run,_t4_idle,_t4_booting,_t4_instr,_t4_set_instr,_t4_halted,_t4_node,_t4_links,_t4_sch_size,_t4_finish \
  -sEXPORTED_RUNTIME_METHODS=callMain,FS,ENV,HEAPU8,cwrap,ccall,stringToUTF8,UTF8ToString \
  $EXTRA -o $OUT
ls -la $OUT
